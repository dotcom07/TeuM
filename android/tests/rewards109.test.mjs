...
test("기본 사용 패턴에서 전체 수집은 약 13.3개월이 걸린다", () => {
  ...
  // 1.0.15 테마 7종(우체국~등대) 98종 추가로 전체 수집은 약 18.6개월이 된다.
  assert.equal(rewardableItemCount, 591);
  assert.equal(completions, 3633);
  assert.ok(Math.abs(collectionMonths - 18.6) < 0.05);
});
