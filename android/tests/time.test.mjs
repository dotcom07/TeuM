...
test("오늘은 그만을 선택하면 업무 종료 시각에 울리지 않고 다음 업무일 슬롯으로 이동한다", () => {
  const mondayAfternoon = at(2024, 1, 1, 14);
  assert.equal(
    nextTickFromWorkStart(endOfWorkToday(mondayAfternoon, base) + 1, base),
    at(2024, 1, 2, 10)
  );
});
